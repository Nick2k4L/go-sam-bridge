...
func TestRawHandler_HandleSend_WithSAM33Options(t *testing.T) {
	...
	// Test with SAM 3.3 options - fully parsed and passed to Send()
	cmd := &protocol.Command{
		Verb:   protocol.VerbRaw,
		Action: protocol.ActionSend,
		Options: map[string]string{
	...
	if mockSess.lastSendOpts.SendTags != 8 {
		t.Errorf("Send() SendTags = %d, want %d", mockSess.lastSendOpts.SendTags, 8)
	}
	if mockSess.lastSendOpts.TagThreshold != 3 {
		t.Errorf("Send() TagThreshold = %d, want %d", mockSess.lastSendOpts.TagThreshold, 3)
	}
	if mockSess.lastSendOpts.Expires != 1800 {
		t.Errorf("Send() Expires = %d, want %d", mockSess.lastSendOpts.Expires, 1800)
	}
...
func TestRawHandler_HandleSend_SAM33OptionsDefaults(t *testing.T) {
	handler := NewRawHandler()
	mockSess := newMockRawSession("test-raw")
...
func TestRawHandler_HandleSend_SAM33OptionsValidation(t *testing.T) {
	handler := NewRawHandler()
